...
export class EventValidator {
    ...
    public static validateLocation(location: string): Result<void, string> {
    ...
    public static validateNewMaxParticipants(
        newMax: number,
        currentMax: number
    ): Result<void, string> {
        if (!newMax || newMax <= 0) {
        ...
        if (newMax < currentMax) {
            return Result.fail("New max participants cannot be less than the current value.");
        }
